...
#[test]
fn repairs_provider_mismatch_across_rollout_and_sqlite() {
    ...
    let execution = execute_repair_plan(codex_home.path(), backups_root.path(), &plan, false)
    ...
    let rollout_content = fs::read_to_string(&rollout_path).expect("read rollout");

    assert_eq!(repaired.model_provider, "mirror");
    assert!(rollout_content.contains("\"model_provider\":\"mirror\""));
    assert_eq!(execution.applied.len(), 3);
}

#[test]
fn repairs_stale_sqlite_rollout_path_with_file_fallback() {
    let codex_home = prepare_codex_home();
    let backups_root = tempdir().expect("create backups root");
    let layout = CodexLayout::from_codex_home(codex_home.path());
    insert_thread(
        &layout.state_db,
        ACTIVE_THREAD_ID,
        &layout.sessions_dir.join("missing-rollout.jsonl"),
        "openai",
        "/workspace/active",
        None,
    );
...
#[test]
fn repairs_archived_thread_recovery() {
    let codex_home = prepare_codex_home();
    let backups_root = tempdir().expect("create backups root");
    let layout = CodexLayout::from_codex_home(codex_home.path());
...
#[test]
fn repairs_config_patch_with_backup_creation() {
    let codex_home = prepare_codex_home();
    let backups_root = tempdir().expect("create backups root");
    ...
    assert_eq!(config.model_provider.as_deref(), Some("mirror"));
    assert!(execution.backup.is_some());
}
