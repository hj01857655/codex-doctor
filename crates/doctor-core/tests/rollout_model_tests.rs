...
fn parses_rollout_file_and_extracts_provider() {
    ...
    let record = RolloutRecord::from_path(&rollout_path, ThreadLocation::Active).expect("parse rollout");
...
fn archived_location_is_represented_in_domain_enum() {
    let dir = tempdir().expect("tempdir");
    let archived_path = dir.path().join("archived_sessions").join("rollout-2026-01-27T12-34-56-00000000-0000-0000-0000-000000000456.jsonl");
    ...
    let record = RolloutRecord::from_path(&archived_path, ThreadLocation::Archived).expect("parse archived rollout");
